...
"""
Fine-tuning the library models for language
modeling on a text file (GPT, GPT-2, BERT, RoBERTa).
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', max_len=128, pad_on_left=False,
        ...
        cached_features_file = os.path.join(cached_features_dir,
                                            'gpt2-large-medium' + '_cached_lm_so_' + str(max_len) + '_' + filename)
        cached_features_we = os.path.join(cached_features_dir,
                                          'gpt2-large-medium' + '_cached_we_so_' + str(max_len) + '_' + filename)
        cached_features_list = os.path.join(cached_features_dir,
                                            'gpt2-large-medium' + '_cached_list_so_' + str(max_len) + '_' + filename)
        ...
        if os.path.exists (cached_features_list) and not args.overwrite_cache:
            ...
            self.neg_examples = []
            self.input_lens =[]
            with open (file_path + "_tpl_generated.tsv", 'r', encoding='utf-8') as data_file:
                import csv
                import sys
