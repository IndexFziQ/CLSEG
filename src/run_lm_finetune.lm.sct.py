...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          ...
                          GPT2Config, GPT2LM_MASKHeadModel, GPT2Tokenizer,
...
MODEL_CLASSES = {
    'gpt2': (GPT2Config, GPT2LM_MASKHeadModel, GPT2Tokenizer),
...
class TextDataset_Line(Dataset):
    def __init__(self, tokenizer, args, file_path='train', max_len=128,
                 pad_on_left=False,
                 pad_token=0):
        assert os.path.isfile(file_path)
        directory, filename = os.path.split(file_path)
        cached_features_file = os.path.join(directory,
                                            args.model_name_or_path + '_cached_lm_' + str(max_len) + '_' + filename)
        cached_features_list = os.path.join(directory,
                                            args.model_name_or_path + '_cached_list_' + str(max_len) + '_' + filename)
        ...
        if os.path.exists (cached_features_list) and not args.overwrite_cache:
            ...
            with open (cached_features_list, 'rb') as handle2:
                self.input_lens = pickle.load(handle2)
        else:
            ...
            with open (file_path, 'r', encoding='utf-8') as csv_file:
                ...
                for line in reader:
                    ...
                    text_pair = ''
                    if line[-1] == '1':
                        text_pair = line[6] + "<|endoftext|>"
                    elif line[-1] == '2':
                        text_pair = line[5] + "<|endoftext|>"
                    text = text_ + text_pair

                    tokenized_text_ = tokenizer.convert_tokens_to_ids (tokenizer.tokenize (text_))
                    tokenized_text = tokenizer.convert_tokens_to_ids (tokenizer.tokenize (text))
                    self.input_lens.append (len (tokenized_text_))
            ...
            logger.info ("Saving features into cached file %s", cached_features_list)
            with open (cached_features_list, 'wb') as handle2:
                pickle.dump(self.input_lens, handle2, protocol=pickle.HIGHEST_PROTOCOL)

    def __len__(self):
        return len(self.examples)

    def __getitem__(self, item):
        return torch.tensor(self.examples[item]), torch.tensor([self.input_lens[item]])


class TextDataset_Block(Dataset):
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    dataset= TextDataset_Line(tokenizer, args, file_path=args.dev_file if evaluate else args.train_file,
...
def train(args, train_dataset, model, tokenizer):
    ...
    for epoch_id in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            mask = make_loss_mask (batch[0], batch[1])
            ...
            model.train()
            outputs = model(inputs, masked_lm_labels=labels) if args.mlm else model(inputs, labels=labels, loss_mask=mask)
